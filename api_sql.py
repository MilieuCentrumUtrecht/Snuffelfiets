...
def call_api():
    ...
    if len(argv) == 3:
        output_file_name = (
            f"{output_map}\\{output_fn}{start_datum}_tot_{stop_datum}.csv"
        )
    else:
    ...
    while True:
        ...
        if len(key) < min_lengte:
            print(
                f"de opgegeven api-key {key} is te kort. Minimale lengte= {min_lengte}"
            )
        elif len(key) > max_lengte:
            print(
                f"de opgegeven api-key {key} is te lang. Maximale lengte= {max_lengte}"
            )
        else:
            break
    ...
    url = (
        "https://ckan-dataplatform-nl.dataplatform.nl/api/3/action/datastore_search_sql"
    )
